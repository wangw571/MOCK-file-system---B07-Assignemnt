    ...
    try {
	    if(overwrite) {
	    	try {
				...
				MOCK.findFile(path).setContent(redirectString.getBytes());
			}
	    }else {
	    	try {
				MOCK.createFileUnderWD(path, redirectString);
			} catch (FileWithSameNameExistedException e) {	
				byte[] existCont = MOCK.findFile(path).getContent();
				...
				{
					newConten[i] = i < existCont.length ? existCont[i] : extenCont[i - existCont.length];
				}
				MOCK.findFile(path).setContent(newConten);
			}
	    }
    } catch (InvalidFileNameException e) {
		CommandObject.logErr("InvalidFileName");
	}
//    try {
//		MOCK.createFileUnderWD(path, redirectString);
//	} catch (FileWithSameNameExistedException e) {
